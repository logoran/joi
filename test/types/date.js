    ...
    describe('validate()', () => {
        ...
        describe('greater', () => {
            ...
            it('errors if .greater("now") is used with a past date', async () => {
                ...
                const message = `"${past}" must be greater than "${dnow}"`;
                expect(err).to.be.an.error(message);
                expect(err.details).to.equal([{
                    message: `"${past}" must be greater than "${dnow}"`,
                    path: [],
                    type: 'date.greater',
                    context: { limit: dnow, label: 'value', key: undefined, value: past }
                }]);
        ...
        describe('less', () => {
            ...
            it('validates less', () => {
                ...
                Helper.validate(Joi.date().less('1-1-1970 UTC'), [
                    ...
                    ['1-1-1970 UTC', false, null, {
                        message: `"${new Date('1-1-1970 UTC')}" ${me}`,
                        details: [{
                            message: `"${new Date('1-1-1970 UTC')}" ${me}`,
                            path: [],
                            type: 'date.less',
            ...
            it('errors if .less("now") is used with a future date', async () => {
                ...
                const message = `"${future}" must be less than "${dnow}"`;
                expect(err).to.be.an.error(message);
                expect(err.details).to.equal([{
                    message: `"${future}" must be less than "${dnow}"`,
                    path: [],
                    type: 'date.less',
                    context: { limit: dnow, label: 'value', key: undefined, value: future }
                }]);
